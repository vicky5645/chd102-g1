  ...
  <section class="custom-tickets">
    <div class="container">
      <div class="des label">
        ...
        <p>
          1.
          請勿離開本頁：在進行客製車票的過程中，請確保不要離開當前頁面，否則可能導致資料丟失。
        </p>
        <p>
          2.
          使用拖曳功能：您可以使用拖曳功能來調整和排列車票上的圖樣和文本。拖曳元素時，請確保放置在適當的位置，以獲得理想的設計效果。
        </p>
        <p>
          3.
          設定圖樣：車票客製功能允許您設定特定的圖樣，請按照指示操作以選擇所需的圖樣。您可以從預設選項中選擇喜愛的圖片。
        </p>
        <p>
          4.
          預覽和確認：在完成設定後，請先預覽車票的外觀和佈局。確保所有內容和圖樣的位置和尺寸符合您的需求。
        </p>
        <p>
          5.
          輸出車票：一旦滿意設計結果，您可以點擊輸出按鈕，以生成最終版本的車票。請注意，一旦車票生成，即無法進一步編輯，請確保在輸出之前檢查所有細節。
        </p>
        <p>
          請牢記這些注意事項，並在使用客製車票功能時謹慎操作。祝您順利創作出理想的車票設計！
        </p>
      </div>
    </div>
  </section>
  <section class="custom-tickets ticket">
    <div class="container">
      ...
      <div class="row make">
        <div
          id="capture"
          class="post-box t-Ani perspective card-shadow"
          @mouseup="onDragEnd"
      ...
      <div class="row">
        <div class="footer">
          <!-- 一些按鈕等元素 -->
          <button id="block-btn" class="btn" name="點我截圖">
            <i class="fa-solid fa-crop-simple"></i>
          </button>
          <button id="download-btn" class="btn" name="下載圖片">
...
export default {
  data() {
    return {
      ...
      // patterns: [
      //   "images/pattern/1.svg",
      //   "images/pattern/2.svg",
      //   "images/pattern/3.svg",
      //   "images/pattern/4.svg",
      // ],
      isDragging: false,
    };
  },
  methods: {
    ...
    async getdataFromMySQL() {
      await axios
        ...
        this.useSvgIcon(svg_icon(".custom-svg", "currentColor"));
    },
    // 操作客製圖片的程式
    useSvgIcon(use) {
      console.log(document.querySelectorAll(".custom-svg").src);
      use
      // 以下是客製圖片的內容------------------------------------------
